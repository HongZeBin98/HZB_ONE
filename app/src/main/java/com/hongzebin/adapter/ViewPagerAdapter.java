...
public class ViewPagerAdapter extends android.support.v4.app.FragmentPagerAdapter {
    ...
    public CharSequence getPageTitle(int position) {
        ...
        if (mFragments.size() == 2) {
            ...
            if (position == 0)
                //为one标题前加图标
                ssb.setSpan(new ImageSpan(OneApplication.getmContext(), R.mipmap.one1), 0, 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            else if (position == 1)
                //为all标题前加图标
                ssb.setSpan(new ImageSpan(OneApplication.getmContext(), R.mipmap.all1), 0, 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            return ssb;
        }
